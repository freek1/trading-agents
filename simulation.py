...
def runSimulation(NUM_AGENTS:int, SCENARIO:str, AGENT_TYPE:str, MOVE_PROB:float, DISTRIBUTION:str, TRADING:bool, SAVE_TO_FILE:bool, RUN_NR:int):
    # Initialize Pygame
    pygame.init()
    ...
    if SAVE_TO_FILE:
        ...
        data = pd.DataFrame({'T': alive_times, 'E': events, 'Scenario': SCENARIO, 'Agent_type': AGENT_TYPE, 'Distribution': DISTRIBUTION, 'Num_agents': NUM_AGENTS, 'Trading': TRADING, 'Move_prob': MOVE_PROB, 'Run_number': RUN_NR})
        ...
        for MOVE_PROB in move_probabilities:
            for TRADING in trading:
                if not TRADING:
                    ...
                    runSimulation(NUM_AGENTS, SCENARIO, AGENT_TYPE, MOVE_PROB, DISTRIBUTION, TRADING, SAVE_TO_FILE, RUN_NR)
                else:
